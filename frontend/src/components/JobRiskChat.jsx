...
export const JobRiskChat = ({ courses = [] }) => {
    ...
    const resetSession = async () => {
        try { if (sessionId) await jobRiskReset(sessionId); } catch { /* swallow */ }
        setSessionId(null);
        setMessages([]);
        setRisk(null);
        setConfidence(0);
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                ...
                <div className="rounded-3xl card-elev overflow-hidden flex flex-col min-h-[540px]">
                    <div className="flex items-center justify-between gap-2 px-4 py-3 border-b border-white/8 bg-[#0B1334]">
                        <div className="flex items-center gap-2">
                            ...
                            <div>
                                <div className="text-sm font-semibold text-white">AI aapki Job legi?</div>
                                <div className="text-xs text-ink-3">Honest risk read, with a way forward.</div>
                            </div>
                        </div>
                        {step === 'chat' && (
                            <button type="button" data-testid={JOBRISK.resetButton} onClick={resetSession} className="inline-flex items-center gap-1.5 text-xs text-ink-3 hover:text-white px-2 py-1 rounded-md hover:bg-white/8">
                    ...
                    <AnimatePresence mode="wait">
                        {step === 'role' ? (
                            <motion.div key="role" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} className="p-5 sm:p-6">
                                <h3 className="font-display text-xl sm:text-2xl font-semibold text-white">Tell us your role. We&apos;ll be honest.</h3>
                                <p className="text-ink-2 mt-1">No alarmist drama. Just a clear, kind risk read — plus a path forward.</p>
                                <input
                                    value={role}
                                ...
                                <div className="mt-3 flex flex-wrap gap-2">
                                    {SAMPLE_ROLES.map((r) => (
                                        <button key={r} type="button" onClick={() => setRole(r)} className="text-xs rounded-full border border-white/12 bg-white/5 hover:bg-white/10 px-3 py-1.5 text-ink-2 hover:text-white">
                                            {r}
                                        </button>
                                    ))}
                                </div>
                            ...
                            <motion.div key="chat" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex-1 flex flex-col">
                                <ChatThread testId={JOBRISK.chatThread} className="min-h-[380px] max-h-[460px]">
                                    {messages.length === 0 && (
                                        <div className="text-sm text-ink-3">
                                            <p>Start by sharing 2-3 tasks you do most days. Be specific (“I write 5 LinkedIn posts and reply to DMs” works great).</p>
                                        </div>
                                    )}
            ...
            <aside className="lg:col-span-5">
                <div className="rounded-3xl card-elev p-5 sm:p-6" style={{ borderTop: `3px solid ${sev.stroke}` }}>
                    ...
                    <div className="mt-6 grid grid-cols-1 gap-5">
                        <div data-testid={JOBRISK.reasons}>
                            <div className="text-xs uppercase tracking-[0.18em] text-ink-3 mb-2 flex items-center gap-1.5">
                            ...
                            <ul className="space-y-2">
                                {(risk?.reasons || []).map((r, i) => (
                                    <li key={i} className="text-sm text-ink-1 flex gap-2">
                                        <span className="shrink-0 w-1.5 h-1.5 rounded-full mt-2" style={{ backgroundColor: sev.stroke, boxShadow: `0 0 6px ${sev.stroke}` }} />
                                        {r}
                                    </li>
                                ))}
                                {!risk && <li className="text-sm text-ink-4">Will appear after first message…</li>}
                            </ul>
                        </div>
                        ...
                        <div data-testid={JOBRISK.safeZones}>
                            ...
                            <ul className="space-y-2">
                                {(risk?.safe_zones || []).map((s, i) => (
                                    <li key={i} className="text-sm text-ink-1 flex gap-2">
                                        <span className="shrink-0 w-1.5 h-1.5 rounded-full mt-2 bg-emerald-400" style={{ boxShadow: '0 0 6px #22C55E' }} />
                                        {s}
                                    </li>
                                ))}
                                {!risk?.safe_zones?.length && <li className="text-sm text-ink-4">Coming up as we chat…</li>}
                            </ul>
                        </div>
                        ...
                        <div data-testid={JOBRISK.recommendations}>
                            ...
                            <div className="space-y-2">
                                ...
                                {!risk?.recommended_courses?.length && <div className="text-sm text-ink-4">Will appear after first message…</div>}
                            </div>
                        </div>
                    </div>
                </div>
