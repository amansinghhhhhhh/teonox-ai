...
import { Send, Loader2, Sparkles, RotateCcw, ArrowRight, Clock, BarChart3, ChevronRight } from 'lucide-react';
...
export const CourseConsultant = ({ courses = [] }) => {
    ...
    const orderedCourses = useMemo(() => {
        ...
        return ranking.map((r) => ({ ...byId.get(r.course_id), match: r })).filter(Boolean);
    }, [courses, ranking]);
    ...
    const resetSession = async () => {
        try { if (sessionId) await consultantReset(sessionId); } catch { /* swallow */ }
        setSessionId(null);
        setMessages([]);
        setRanking([]);
        setStep('audience');
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <section className="lg:col-span-7">
                <div className="rounded-3xl card-elev overflow-hidden flex flex-col min-h-[540px]">
                    <div className="flex items-center justify-between gap-2 px-4 py-3 border-b border-white/8 bg-[#0B1334]">
                        ...
                        {step === 'chat' && (
                            <button type="button" onClick={resetSession} className="inline-flex items-center gap-1.5 text-xs text-ink-3 hover:text-white px-2 py-1 rounded-md hover:bg-white/8">
                    ...
                    <AnimatePresence mode="wait">
                        {step === 'audience' ? (
                            ...
                            >
                                <ChatThread testId={COURSES.chatThread} className="min-h-[380px] max-h-[460px]">
                                    {messages.length === 0 && (
                                        <div className="text-sm text-ink-3">
                                            <p className="mb-3">Hi! Tell me about your goal. Try one of these or type your own:</p>
            ...
            <aside className="lg:col-span-5">
                <div className="flex items-center justify-between mb-3">
                    <h3 className="font-display text-lg font-semibold text-white">Recommended courses</h3>
                    <Badge className="bg-white/5 text-ink-2 border border-white/10 hover:bg-white/5">Live re-ranks</Badge>
                </div>
                            ...
                            return (
                                ...
                                >
                                    <div className="flex items-start justify-between gap-3">
                                        <div>
                                            <div className="text-base sm:text-lg font-display font-semibold text-white leading-snug">{c.title}</div>
                                            <div className="text-xs text-ink-3 mt-0.5">{c.subtitle}</div>
                                        </div>
                                        {match && <MatchBadge percentage={match.match_percentage} label={match.label} />}
                                    </div>
                                    ...
                                    <div className="mt-3 flex items-center gap-3 text-xs text-ink-2">
                                        ...
                                        <span className="ml-auto inline-flex items-center gap-2">
                                            <span className="text-white font-semibold">₹{c.price_inr.toLocaleString('en-IN')}</span>
                                            <span className="line-through text-ink-4">₹{c.original_price_inr.toLocaleString('en-IN')}</span>
                                        </span>
                                    </div>

                                    {match?.reason && (
                                        <p className="mt-3 text-xs text-ink-2 bg-white/4 rounded-lg p-2.5 border border-white/8">
                                            <span className="font-semibold text-white">Why:</span> {match.reason}
                                        </p>
                                    )}
